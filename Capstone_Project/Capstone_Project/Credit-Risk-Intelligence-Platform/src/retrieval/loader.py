...
class DocumentLoader:
    """Extracts clean text from PDF documents with automatic OCR fallback."""
    ...
    def load(self, file_path: str | Path) -> tuple[str, int]:
        ...
        # Step 1 — PyMuPDF (primary, fastest)
        try:
    ...
    def _load_with_ocr(self, path: Path) -> tuple[str, int]:
        ...
        if not settings.OCR_ENABLED:
            logger.info("OCR disabled in settings. Skipping.")
            return "", 0
        ...
        try:
            ...
            for i, image in enumerate(images, start=1):
                page_text = pytesseract.image_to_string(image, lang="eng", config="--psm 3")
                if page_text.strip():
            ...
            logger.error(
                ...
                "Then install Tesseract from TESSERACT_INSTALL.md",
                exc,
            )
            return "", 0
        except Exception as exc:
            logger.error("Tesseract OCR failed for %s: %s", path.name, exc)
            return "", 0
