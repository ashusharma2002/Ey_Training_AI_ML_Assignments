# src/ui/sidebar.py  — Enterprise Redesign v2.1 (Radio-based nav, no white boxes)
# ============================================================
# Uses st.radio() styled via CSS — no invisible buttons.
# Version-stable across all Streamlit releases.
# ============================================================
from __future__ import annotations
import streamlit as st
from config.settings import settings
from src.models.schemas import PlatformState, ProcessingStatus
from src.ui.theme import C

# Page order must match _GROUPS below
_PAGES = [
    "📊  Dashboard",
    "📤  Upload Documents",
    "📋  Document Summary",
...
# Map display label → clean page name (strip icon prefix)
_LABEL_TO_NAME = {p: p[3:] for p in _PAGES}
_NAME_TO_LABEL = {v: k for k, v in _LABEL_TO_NAME.items()}


def render(state: PlatformState) -> str:
    """Render the sidebar. Returns the selected clean page name."""
    ...
    # Persist and return clean page name
...
def _nav(current_label: str) -> str:
    ...
    # Insert ANALYSIS group label between index 2 and 3
    # We render it as a disabled markdown between the radio widget split
    # All 6 items in ONE radio so keyboard nav works
    selected = st.radio(
        "navigation",
        options=_PAGES,
        index=idx,
...
def _status(state: PlatformState) -> None:
    ...
    rows = [
        ...
        ("💡", "Recs",
         "Ready" if state.recommendations else "Pending", ""),
        ("🤖", "Chat",
...
def _provider() -> None:
    ...
    dot_color = "#00875A" if settings.LLM_PROVIDER == "openai" else "#4285F4"  # green=OpenAI, blue=Gemini
    st.markdown(
